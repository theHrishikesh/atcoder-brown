...
#define ascend(x) sort(all(x))
#define descend(x) sort(all(x),greater<>())
// ----------------------------------------------------------------------------------------
...
ll Comb(ll n, ll r, ll MOD = 998244353) {
    static vll fact = {1}, invfact = {1};
    static ll prepared = 0;

    if (r < 0 || r > n) return 0;

    // Expand factorials if needed
    if (prepared < n) {
        int old = prepared;
        fact.resize(n + 1);
        invfact.resize(n + 1);
        ...
        ll e = MOD - 2;
        while (e) {
            if (e & 1) invfact[n] = invfact[n] * pw % MOD;
            pw = pw * pw % MOD;
            e >>= 1;
        }
...
int main() 
{
    ...
    while (val < n*n)
    {
        if (x + dx4[dir % 4] < n && x + dx4[dir % 4] >= 0 && y + dy4[dir % 4] < n && y + dy4[dir % 4] >= 0 && grid[x + dx4[dir % 4]][y + dy4[dir % 4]] == LLONG_MAX)
        {

            grid[x + dx4[dir % 4]][y + dy4[dir % 4]] = ++val;
            x += dx4[dir % 4];
            y += dy4[dir % 4];
        }
        else dir++;

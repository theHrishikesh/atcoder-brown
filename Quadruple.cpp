...
{
    sll(n,k);
    //subproblem a + b = s ; find total pairs
    // 1 <= a <= n;
    // 1 <= b <= n; 1 <= s - a <= n; 1 - s <= -a <= n - s; s - n <= a <= s - 1;
    // max(1, s - n) <= a <= min(n, s - 1);
    // total no of soln = max(1, s - n) <= a <= min(n, s - 1);

    // in this problem a + b = x && c + d = y;
    // total number of ways of forming x and total number of ways of forming y

    auto f = [&](ll n, ll s) {
        return max(0LL,min(n, s - 1) - max(1LL, s - n) + 1LL);
    };

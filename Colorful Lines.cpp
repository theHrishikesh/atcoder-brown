...
int main()
{
    ...
    rrep(q)
    {
        if (t[i] == 1)
        {
            if (color_row.count(n[i]) > 0) continue;
            ans[col[i]] += w - color_col.size();
            color_row.insert(n[i]);
        }
        else
        {
            if (color_col.count(n[i]) > 0) continue;
            ans[col[i]] += h - color_row.size();
            color_col.insert(n[i]);
        }
    }

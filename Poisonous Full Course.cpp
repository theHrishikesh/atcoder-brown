...
int range_count(int lq, int rq, long long low, long long high) const {
        ...
        return l->range_count(pref[lq], pref[rq], low, high) + r->range_count(lq - pref[lq], rq - pref[rq], low, high);
    }
};
...
int main()
{
    ...
    rep(i,1,n + 1)
    {
        ...
        if (x[i] == 0)
        {
            if (dp[i - 1][0] != -INF) dp[i][0] = max(dp[i][0],dp[i - 1][0] + y[i]);
            if (dp[i - 1][1] != -INF) dp[i][0] = max(dp[i][0],dp[i - 1][1] + y[i]);
        }
        else
        {
            if (dp[i - 1][0] != -INF) dp[i][1] = max(dp[i][1],dp[i - 1][0] + y[i]);
        }
    }
    out(max(dp[n][1],dp[n][0]));

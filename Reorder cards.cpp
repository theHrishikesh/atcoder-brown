...
int main() // teaches techniques of building compression maps;
{
    ll h, w, n;
    cin >> h >> w >> n;
    ...
    rows.erase(unique(begin(rows),end(rows)),end(rows));

    sort(begin(cols),end(cols));
    cols.erase(unique(begin(cols),end(cols)),end(cols));

    unordered_map<ll,ll> row_id , col_id;
    ...
    for (ll i = 0; i < n; i++)
    {
        cout << row_id[a[i]] << " " << col_id[b[i]]<< endl;
    }

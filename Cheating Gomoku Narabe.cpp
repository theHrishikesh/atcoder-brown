...
struct SCC {
    ...
    vvll scc() {
        ...
        reverse(all(comps)); // topological order (AtCoder compatible)
        return comps;
    }
};
...
int main()
{
    ...
    rep(i,0,h)
    {
        vll cnt_d(w + 1,0), cnt_x(w + 1,0);
        rep(j,0,w)
        {
            cnt_d[j + 1] = cnt_d[j] + (g[i][j] == '.');
            cnt_x[j + 1] = cnt_x[j] + (g[i][j] == 'x');
        }

        rep(j,0,w - k + 1)
        {
            if (cnt_x[j + k] - cnt_x[j] > 0) continue;
            chmin(ans, cnt_d[j + k] - cnt_d[j]);
        }
    }
    ...
    rep(i,0,w)
    {
        vll cnt_d(h + 1,0), cnt_x(h + 1,0);
        rep(j,0,h)
        {
            cnt_d[j + 1] = cnt_d[j] + (g[j][i] == '.'); // FIX
            cnt_x[j + 1] = cnt_x[j] + (g[j][i] == 'x'); // FIX
        }

...
int main() {
    ...
    // dp[i][0] = cost to type first i chars, end with Caps OFF
    // dp[i][1] = cost to type first i chars, end with Caps ON
    vector<vector<ll>> dp(n + 1, vector<ll>(2, INF));

    dp[0][0] = 0;
    dp[0][1] = z;  // toggle before starting (optional but clean)

...
int main() 
{
    ...
    rep(i,0,h) {
        sstr(s);
        g.pb(voc(s));   // voc(s) -> vector<char> of length w
    }

    vvbl vis(h, vbl(w, false));

    rep(i,0,h)
    {
        rep(j,0,w)
        {
            // Start only on an unvisited black cell
            if (g[i][j] == '#' && !vis[i][j])
            {
                ...
                while (!stk.empty())
                {
                    ...
                    ll cx = cur[0], cy = cur[1];
                    cc++;

                    // use k so we do not shadow i
                    rep(k,0,4)
                    {
                        ll nx = cx + dx4[k], ny = cy + dy4[k];
                        if (0 <= nx && nx < h && 0 <= ny && ny < w && g[nx][ny] == '#' && !vis[nx][ny])
                        {
                            vis[nx][ny] = 1;
                            stk.push({nx,ny});
                        }
